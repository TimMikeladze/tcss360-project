...
public class TopPane extends GenericPane<StackPane> implements EventHandler<ActionEvent>,
        ProgressSpinnerCallbacks, Updatable {
    ...
    private void create() {
        ...
        welcomeLabel = new Label(LoggedUser.getInstance().getUser().getFullName());
        ...
        createConferenceButton = new Button("New Conference");
        createConferenceButton.setVisible(false);
        createConferenceButton.setOnAction(this);
        rightBox.getChildren().add(createConferenceButton);
    ...
    public void handle(final ActionEvent event) {
        if (event.getSource() == createConferenceButton) {
            mainPaneCallbacks.changeCenterPane(new CreateConferencePane(callbacks, mainPaneCallbacks, this));
    ...
    @Override
    public void update() {
        setCreateConferenceButtonVisible(false);
    }
}
