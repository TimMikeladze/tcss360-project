...
public class PaperPane extends GenericPane<GridPane> implements EventHandler, AddUserCallback {
    ...
    private void create() {
        ...
        if (Permissions.hasPermission(PaperManager.class, "recommendPaper", LoggedUser.getInstance().getPermissions())
        ...
        if (paper.getAuthorID() == LoggedUser.getInstance().getUser().getID()
                || Permissions.hasPermission(ReviewManager.class, "submitReview", LoggedUser.getInstance()
                        .getPermissions())) {
            bottomBox.getChildren().add(downloadPaperButton);
        }
        
        if (Permissions.hasPermission(ReviewManager.class, "acceptPaper", LoggedUser.getInstance().getPermissions())) {
            bottomBox.getChildren().add(acceptRejectPaperButton);
        }
        
        if (Permissions.hasPermission(ReviewManager.class, "submitReview", LoggedUser.getInstance().getPermissions())) {
            bottomBox.getChildren().add(submitReviewButton);
        }
        
        if (Permissions.hasPermission(ReviewManager.class, "assignPaper", LoggedUser.getInstance().getPermissions())) {
    ...
    public void handle(final Event event) {
        ...
        if (source == paperReviewsTable) {
            ...
            new AddReviewersPane(paper.getConferenceID(), sceneCallback.getPrimaryStage(), progressSpinnerCallback,
                    this).showDialog();
    ...
    private class RemovePaperService extends ProgressSpinnerService {
        ...
        protected Task<String> createTask() {
            return new Task<String>() {
                ...
                protected String call() {
                    try {
                        PaperManager.removePaper(paperID, LoggedUser.getInstance().getUser().getID());
                        setSuccess(true);
                    }
                    catch (Exception e) {
                        //TODO make sure message dialog works
