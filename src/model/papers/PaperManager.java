...
public class PaperManager {
	...
	public static void submitPaper(int conferenceID, int authorID,
		...
		if (MAX_PAPERS > getNumberOfSubmittedPapers(conferenceID, authorID)) {
			Database.getInstance()
					...
					.addParameter("file", convertedFile)
	...
	@Permission(level = 400)
	public static void assignPaperToSubprogramChair(final int paperID,
			final int userID) {
		Database.getInstance()
				.createQuery(
	...
	public static List<Review> getReviews(final int conferenceID,
			final int authorID) {
		return Database
				.getInstance()
				.createQuery(
	...
	public static void submitReview(int paperID, int reviewerID, File file)
		...
		Database.getInstance()
				...
				.addParameter("file", convertedFile)
				.addParameter("fileExtension", fileExtension).executeUpdate();
	}
	@Permission(level = 400)
