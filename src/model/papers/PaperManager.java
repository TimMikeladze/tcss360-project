...
public class PaperManager {
    ...
    private static List<Paper> getAssignedPapers(final int conferenceID, final int userID,
        ...
        return Database
                .getInstance()
                .createQuery(
                        "SELECT p.ConferenceID, p.ID AS PaperID, p.Title, p.Description, p.AuthorID, p.SubmissionDate, p.Status, p.Revised, p.FileExtension, CONVERT(p.File USING utf8) AS File, p.RevisionDate, Recommended, CONCAT(u.Firstname, ' ', u.Lastname) AS Username FROM papers AS p JOIN assigned_papers AS a ON a.PaperID = p.ID JOIN users AS u ON u.ID = p.AuthorID WHERE p.ConferenceID = :conferenceID AND a.UserID = :userID AND a.PermissionID = :permissionID")
    ...
    public static List<Paper> getAssignedPapersForUserInConference(final int userID,
        ...
        return Database
                ...
                .addParameter("userID", userID).addParameter("conferenceID", userID)
                .executeAndFetch(Paper.class);
    }
    ...
    public static List<Paper> getPapers(final int conferenceID) {
        return Database
                .getInstance()
                .createQuery(
                        "SELECT p.ConferenceID, p.ID AS PaperID, p.Title, p.Description, p.AuthorID, p.SubmissionDate, p.Status, p.Revised, p.FileExtension, CONVERT(p.File USING utf8) AS File, p.RevisionDate, p.Recommended, CONCAT(u.Firstname, ' ', u.Lastname) AS Username FROM papers AS p JOIN users AS u ON u.ID = p.AuthorID WHERE p.ConferenceID = :conferenceID")
    ...
    public static List<Paper> getAuthorsSubmittedPapers(final int authorID,
        ...
        return Database
                ...
                .addParameter("authorID", authorID).addParameter("conferenceID", conferenceID)
                .executeAndFetch(Paper.class);
    ...
    public static List<Paper> getAuthorsSubmittedPapers(final int authorID) {
        return Database
                .getInstance()
                .createQuery(
                        "SELECT p.ConferenceID, p.ID AS PaperID, p.Title, p.Description, p.AuthorID, p.SubmissionDate, p.Status, p.Revised, p.FileExtension, CONVERT(p.File USING utf8) AS File, p.RevisionDate, p.Recommended, CONCAT(u.Firstname, ' ', u.Lastname) AS Username FROM papers AS p JOIN users AS u ON u.ID = p.AuthorID WHERE p.AuthorID = :authorID")
